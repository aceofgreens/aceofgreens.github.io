...
**Multiview SFM**. How do we reconstruct structure from multiple views? The classic approach is cumbersome. It involves incrementally adding more and more images. We start with some pair of images for which we find the camera poses and the point cloud. Then incrementally add a new image, solve for its camera pose using [PnP](https://en.wikipedia.org/wiki/Perspective-n-Point), triangulate the new 3D points, and periodically do bundle adjustment, refining all camera poses and all 3D points.
...
- Triangulation is done by another transformer that takes in the tracked points and a predefined point cloud which is refined.
