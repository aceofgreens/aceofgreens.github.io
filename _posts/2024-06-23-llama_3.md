...
In practice, suppose there are $H$ attention heads and define $D' = D/H$. We create the $\Theta$ array and a regular array $\text{m} = \\{0, 1, \ldots, 2S-1\\}$. Then, $\text{m} \  \Theta^{T}$ is of shape $(2S, D'/2)$ and contains the rotation angles for every possible relative position and every dimension. We then create a complex-valued tensor where elment is a complex exponential with unit magnitude and angle given by $\text{m} \  \Theta^{T}$. This tensor, let's call it $P$, represents the angles to rotate by.
