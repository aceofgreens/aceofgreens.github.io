...
Self-supervised learning (SSL) has emerged as the cornerstone behind the surge of large-scale models permeating the mainstream. In this post, we'll delve into the main familites of SSL algorithms, primarily with a focus on those in the realm of computer vision.
...
BEiT is a complex model. MAE (masked autoencoders) simplify it by directly reconstructing image patches instead of discrete tokens. MAE randomly selects visible image patches, passes them to an encoder, adds mask tokens and uses a lightweight decoder to reconstruct the whole image. **SimMIM** also goes in the direction of simplifying the pipeline by having a single encoder and a single-layer prediction head.
