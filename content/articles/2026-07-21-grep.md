...
GNU `grep` uses Boyer-Moore for a single fixed pattern and Aho-Corasick for multiple fixed patterns. For general regular-expressions it uses automata and delegates features such as backreferences to a slower matcher [1]. The command-line switch `-F` is therefore more than a request for different punctuation. It tells the program a stronger fact about the problem, and strong facts allow better algorithms.
